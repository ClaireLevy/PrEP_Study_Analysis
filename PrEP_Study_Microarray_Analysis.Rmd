...
cross_study_camera_function<-function(expressedProbes, Condition){
...
#make the index of the geneSet using identifiers from my eSet
index <- ids2indices(gene.sets = up_and_down_gsets, identifiers = identifiers)
...
#use mapply to iterate over both the eSets vector and the conditions vector, SIMPLFY = FALSE or else it makes it into a weird matrix thing.
x<-mapply(FUN = cross_study_camera_function, expressedProbes = eSets, Condition  = conds, SIMPLIFY = FALSE)
...
get_exprs_for_cross_study_analysis<- function(expressedProbes, Tissue){
  ...
  mutate(Visit = ifelse(str_detect(Sample, "Enrollment"), "Baseline", "60 days"))%>%
  mutate(Treatment = ifelse(str_detect(Sample, "Enrollment"), "Baseline", "Truvada"))%>%
  mutate(TreatmentLocation = ifelse(str_detect(Sample, "Enrollment"), "Baseline", "Oral"))%>%
  mutate(TreatmentInterval = ifelse(str_detect(Sample, "Enrollment"), "Baseline", "Daily"))%>%
  mutate(TreatmentLength = ifelse(str_detect(Sample, "Enrollment"), "Baseline", "60 days"))
...
all_exprs<-mapply(get_exprs_for_cross_study_analysis, expressedProbes = eSets, Tissue = tissue, SIMPLIFY = FALSE)

PrEP_filtered_expression_values<-bind_rows(all_exprs)

save(PrEP_filtered_expression_values, file = "../../Tenofovir R01/Cross-StudyMicroarrayAnalysis/data-in/PrEP-filtered-expression-values.Rda")

save(PrEP_filtered_expression_values,file = "R_output/PrEP-filtered-expression-values.Rda")
